...
static const char* fascia_to_str(FasciaOraria f){
    switch(f){
        case MATTINA:    
            return "Mattina    (08:00-13:00)";
        case POMERIGGIO: 
            return "Pomeriggio (13:00-18:00)";
        case SERA:       
            return "Sera       (18:00-23:00)";
        default:         
            return "Sconosciuta";
    }
}
...
static void op_prenota(const char *matricola){
    ...
    fascia = leggi_fascia();
    if((int)fascia < 0){ 
        printf("[ERRORE] Fascia non valida.\n"); 
        return; 
    }
    ...
    if(p == NULL){ 
        libera_posto_aula(fascia, posto); 
        return; 
    }
    set_posto_prenotazione(p, posto);
    aggiungi_prenotazione(lista_prenotazioni, p);
...
static void op_checkin(const char *matricola){
    FasciaOraria fascia;
    Prenotazione p;
...
static void op_checkout(const char *matricola){
    FasciaOraria fascia;
    Prenotazione p;
    ...
    if(p == NULL || prenotazione_get_stato(p) != CHECKED_IN){
        printf("[ERRORE] Studente non risulta presente in aula per questa fascia.\n");
        return;
    }
    libera_posto_aula(fascia, prenotazione_get_posto(p));
...
static void menu_studente(void){
    ...
    do {
        ...
        printf("  3. Check-in\n");
        printf("  4. Check-out\n");
        printf("  5. Annulla una prenotazione\n");
        printf("  6. Visualizza le mie prenotazioni\n");
        ...
        switch(scelta){
            ...
            case 9:
                ...
                while(!is_empty(coda_attesa)){
                    ...
                    enqueue(tmp, queue_peek_matricola(coda_attesa),
                            queue_peek_fascia(coda_attesa),
                            queue_peek_tipo(coda_attesa));
                    dequeue(coda_attesa);
                }
                while(!is_empty(tmp)){
                    enqueue(coda_attesa, queue_peek_matricola(tmp),
                            queue_peek_fascia(tmp),
                            queue_peek_tipo(tmp));
                    dequeue(tmp);
                }
